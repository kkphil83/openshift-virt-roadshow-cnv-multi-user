...
[NOTE]
====
**Migration Toolkit for Virtualization (MTV)**는 OpenShift Virtualization과는 별개의 도구이며, *Virtualization* 관점에서는 접근할 수 없습니다.
====

. 왼쪽 메뉴에서 *Virtualization* 클릭 후 드롭다운에서 **Administrator**를 선택합니다.
+
image::2025_spring/module-02-mtv/05_Admin_Persona.png[link=self, window=blank, width=100%]

. 왼쪽 메뉴에서 *Migration* -> **Providers for virtualization**으로 이동합니다.
...
기본적으로 **OpenShift Virtualization**을 대상 플랫폼으로 나타내느 **host**라는 프로바이더가 있습니다.
