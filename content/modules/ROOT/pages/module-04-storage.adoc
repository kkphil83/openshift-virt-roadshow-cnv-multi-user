...
특히 OpenShift 가상화에 특화된 스토리지 프로토콜(예: NFS, iSCSI, FC 등)에는 제한이 없습니다. 클러스터 내 VM의 라이브 마이그레이션을 지원하기 위해 *RWX 액세스 모드* 가 사용 가능해야 한다는 점만 요구됩니다. 그 외에는 조직의 VM 및 애플리케이션 요구 사항에 가장 적합한 스토리지를 선택하면 됩니다.
...
. 먼저 왼쪽 메뉴에서 *스토리지* -> *영구 볼륨 클레임* 을 클릭합니다. *vmexamples-{user}* 네임스페이스에 있는지 확인하세요. 이전 섹션에서 *fedora01* 가상 머신을 생성할 때 만들어진 *fedora01* PVC가 표시될 것입니다.
+
...
. `Virtualization` 드롭다운 메뉴로 돌아가서 왼쪽 메뉴에서 `가상 머신` 메뉴를 클릭합니다. 가운데 열에서 *vmexamples-{user}* 프로젝트를 확장하고 *fedora01* 가상 머신을 선택합니다.
+
...
+
NOTE: 스냅샷에 *cloudinitdisk* 가 포함되지 않는다는 경고 메시지가 표시됩니다. 이는 초기 부팅에 사용되는 임시 디스크이기 때문에 발생하는 정상적인 현상입니다.
...
. `스냅 샷` 타일에서 스냅샷 옆에 있는 점 3개 메뉴를 클릭하면 VM이 중지된 상태에서 `복원` 이 활성화된 것을 확인할 수 있습니다. `복원` 을 클릭하세요.
+
...
. *동작* 메뉴에서 `복제` 를 누르면 대화 상자가 열립니다. 복제된 VM의 이름을 *fedora02* 로 지정하고 `VirtualMachine을 생성 후 시작` 확인란이 선택 해제되어 있는지 확인한 다음 `복제` 를 클릭합니다.
+
image::2025_spring/module-04-storage/18_VM_Clone_Dialog.png[link=self, window=blank, width=100%]

. 새로운 VM이 생성되고 디스크가 복제되면 포털에서 자동으로 새 VM으로 리디렉션되며, *작성* 시간이 매우 최근으로 표시되는 것을 확인할 수 있습니다.
+
image::2025_spring/module-04-storage/19_VM_Cloned.png[link=self, window=blank, width=100%]
+
IMPORTANT: 복제된 VM은 원본 VM과 동일한 ID를 가지게 되므로 VM과 상호 작용하는 애플리케이션 및 기타 클라이언트와 충돌이 발생할 수 있습니다. 외부 네트워크에 연결된 VM이나 동일 프로젝트 내의 VM을 복제할 때는 주의해야 합니다.

. 화면 상단의 `YAML` 메뉴를 클릭하면 VM 이름이 *fedora02* 인 것을 확인할 수 있습니다. 하지만 *fedora01* 원본 VM에서 남은 일부 레이블은 수동으로 업데이트해야 합니다.
+
image::2025_spring/module-04-storage/20_Cloned_VM_YAML.png[link=self, window=blank, width=100%]

. YAML 파일에서 *app* 과 *kubevirt.io/domain* 값을 *fedora02* 로 변경한 다음 하단의 `저장` 버튼을 클릭하세요. *fedora02* 가 새 버전으로 업데이트되었다는 메시지가 표시됩니다. 이렇게 하면 이후 모듈에서 이 VM을 사용할 때 발생할 수 있는 문제를 방지할 수 있습니다.
+
image::2025_spring/module-04-storage/21_Cloned_VM_YAML_Saved.png[link=self, window=blank, width=100%]

. 가상 머신의 YAML 파일 수정이 완료되면 가상 머신을 시작하여 *fedora01* 과 *fedora02* 가 모두 실행되도록 하십시오.
+
image::2025_spring/module-04-storage/22_Fedora02_Running.png[link=self, window=blank, width=100%]

== 요약

이번 실습에서는 가상 머신 관리 시 사용할 수 있는 스토리지 옵션을 살펴보았습니다. 또한 가상 머신에 할당된 스토리지에 따라 필요한 여러 VM 관리 기능, 예를 들어 기본 복원을 위한 VM 스냅샷 생성, 다른 프로젝트에서 사용하거나 향후 개발을 간소화하기 위한 VM 복제 등을 수행했습니다.
